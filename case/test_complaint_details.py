...
sql = "select * from test_datas where menuName='投诉管理' and caseName='详情'"
datas = common.dbHelper.selectMethod(sql)

@ddt.ddt
class TestComplaintDetails(unittest.TestCase):
    '''投诉详情页面'''

    def setUp(self):
        self.proxies = {'http': 'http://localhost:8888', 'https': 'http://localhost:8888'}
        self.s = requests.session()
        self.url = "http://192.168.0.142:9099/rest/complaintDetails"
    ...
    @ddt.data(*datas)
    def test_complaint_details(self, data):
        '''投诉列表用例'''
